...
class OptionsController extends Controller
{
    ...
    public function index(){
        $options = Option::where('status', 1)->with(['optionValues' => function ($query) {
    ...
    public function addOptionsValues(Request $request){
    ...
    public function editOptions(Request $request, $type, $id){
        if ($type == 'parent') {
            $option = Option::find($id);
            $name = $request->name;
        ...
        if ($type == 'child') {
            ...
            $optionValue->option_id     = $request->parentId;
            $optionValue->option_value  = $request->name;
            $optionValue->status        = $request->status;
            $optionValue->save();
        }
    }

    public function destroyOptions(Request $request, $type, $id){
        if ($type == 'parent') {
